...
{
	T_Vertex out;
	if((float)viewportSize[0]/(float)viewportSize[1] > (float)texSize[0]/(float)texSize[1])
	{
		out.position = float4(vertices[4*indices[vertexID]] * ((float)texSize[0]*(float)viewportSize[1])/((float)texSize[1]*(float)viewportSize[0]), vertices[4*indices[vertexID]+1], 0, 1); 
	}
	else
	{
		out.position = float4(vertices[4*indices[vertexID]], vertices[4*indices[vertexID]+1] * ((float)texSize[1]*(float)viewportSize[0])/((float)texSize[0]*(float)viewportSize[1]), 0, 1); 
	}
